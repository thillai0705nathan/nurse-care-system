...
function getFilteredNurses() {
  ...
  let result = allNurses.filter((n) => {
    ...
    return matchesQuery && matchesGender && matchesNationality && matchesDuty;
  });
...
function buildNurseCard(nurse) {
  ...
  card.innerHTML = `
    ...
    <div class="nurse-card-side">
      <span class="badge badge-rating">★ ${getRating(nurse)}</span>
...
function openNurseDetail(nurseId) {
  ...
  dom.nurseDetailRating.innerHTML = `<span class="star-rating">★ ${getRating(nurse)}</span> &nbsp;(${getReviewCount(nurse)} reviews)`;
