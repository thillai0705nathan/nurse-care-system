/**
 * ============================================================
 * NURSE MANAGEMENT SYSTEM — Admin Sign Up Logic
 * File: admin-signup.js
 * Front-end only for now: validates the form and stores the
 * new admin account in localStorage ("nms_admin_accounts") so
 * admin-login.js can authenticate against it. Will be replaced
 * by real Firebase/Java backend auth later.
 * ============================================================ */
...
const dom = {
  ...
  inviteCodeGroup:     document.getElementById('inviteCodeGroup'),
...
function validateInviteCode() {
  const val = dom.inviteCode.value.trim();
  if (!val) {
    setFieldError(dom.inviteCodeGroup, dom.inviteCodeError, 'Invite code is required.');
    return false;
  }
  if (val.toUpperCase() !== CONFIG.INVITE_CODE) {
    setFieldError(dom.inviteCodeGroup, dom.inviteCodeError, 'Invalid invite code.');
    return false;
  }
  setFieldValid(dom.inviteCodeGroup, dom.inviteCodeError);
  return true;
}
...
async function handleSignupSubmit(e) {
  ...
  try {
    await api.post('/auth/admin/signup', {
      ...
      inviteCode: dom.inviteCode.value.trim(),
    });
...
function init() {
  ...
  [
    ...
    [dom.inviteCode, dom.inviteCodeGroup, dom.inviteCodeError],
  ...
  console.info('%c[NMS] Admin signup module initialised.', 'color: #E91E63; font-weight: bold;');
}
