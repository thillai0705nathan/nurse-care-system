...
function prefillFormForEdit(nurse) {
  ...
  if (nurse.skills) skillsChipApi.setItems(nurse.skills.split(',').map((s) => s.trim()).filter(Boolean));
...
async function init() {
  ...
  skillsChipApi = createChipInput(dom.skillsInput, dom.skillsChips, dom.skillsHidden);
